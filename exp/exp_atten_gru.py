...
import warnings
from models.attn import FullAttention, ProbAttention, AttentionLayer, CarAttention
...
class Exp_Informer(Exp_Basic):
    ...
    def predict(self, setting, load=False):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pred_loader):
            ...
            # pred, true = self._api_one_batch(pred_data, batch_x, batch_y, batch_x_mark, batch_y_mark)
            # loss = criterion(pred, true)
            print(f'<-----------step:{i}----{model_file}------>')
            # res = pred_data.scaler.inverse_transform(batch_x, True)
            # last_x, last_y = res[0, 71, 8:10].cpu().detach().numpy()
    ...
    def _api_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark,
